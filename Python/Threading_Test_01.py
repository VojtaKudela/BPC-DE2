...
class SerialGUI:
    ...
    def send_message(self):
        if self.serial_connection and self.serial_connection.is_open:
            ...
            if message:
                try:
                    self.serial_connection.write(message.encode('ascii') + b'\n')  # Send the message
                    self.text_area.insert(tk.END, f"Sent: {message}\n")
                    self.message_entry.delete(0, tk.END)  # Clear the entry field
                except Exception as e:
                    self.text_area.insert(tk.END, f"Error sending message: {e}\n")
            else:
                self.text_area.insert(tk.END, "No message to send.\n")
        else:
            self.text_area.insert(tk.END, "Serial connection not open.\n")
